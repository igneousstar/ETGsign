...
class vertBrightSel extends UIobject{
  /**
  * name of the Slider
  */
  private String name;
  ...
  */
  ...
  public vertBrightSel(String name, int centerX, int centerY, int sliderLength, int sliderWidth){
  ...
  */
  void drawUI(){
    ...
    for (float j = 0; j < sliderWidth; j++) {
      stroke(primeColor, secondColor, map(i, 0, sliderLength, 255, 0));
  ...
  rectMode(CENTER);
    ...
    value[0] = map(barPos[1], yMin, yMax, mapMax, mapMin);
